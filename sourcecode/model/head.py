...
def build_head(head_cfg):
    ...
    elif head_cfg.type == 'MultiLayerCombine':
        return MultiLayerFloorHead(head_cfg)

BatchNorm = nn.BatchNorm2d

class MultiLayerFloorHead(nn.Module):
    ...
    decoder_module = []
    self.last_layer_room = nn.Sequential(
        nn.Conv2d(
        ...
        nn.Conv2d(
            in_channels=last_inp_channels,
            out_channels=last_inp_channels,
            kernel_size=self.final_conv_kernel//2,
            stride=1,
            padding=1 if self.final_conv_kernel == 3 else 0))

    self.last_layer_wall = nn.Sequential(
        nn.Conv2d(
            in_channels=last_inp_channels,
            out_channels=last_inp_channels,
            kernel_size=1,
